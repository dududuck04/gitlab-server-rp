...
UBUNTU_AMI_ID=ami-09a7535106fbd42d5
...
aws ec2 run-instances \
  ...
  --subnet-id ${SUBNET_ID} \
  --security-group-ids ${GITLAB_SG} \
  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=gitlab-aws-cli-generate}]" \
  --block-device-mappings 'DeviceName=/dev/sda1,Ebs={VolumeSize=40}' \
  --user-data file://userdata-without-docker.txt
